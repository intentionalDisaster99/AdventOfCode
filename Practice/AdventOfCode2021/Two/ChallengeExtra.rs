...
fn main() {
    ...
    for (_, &item) in data.iter().enumerate() {
        ...
        if item.contains("forward") {
            // Increasing the horizontal position by x units
            pos += item[8..].trim().parse::<usize>().unwrap();

            // Increasing the position by the aim
            depth += aim * item[8..].trim().parse::<usize>().unwrap();
        } else if item.contains("down") {
            aim += item[5..].trim().parse::<usize>().unwrap();
        } else {
            aim -= item[3..].trim().parse::<usize>().unwrap();
        }
    }
