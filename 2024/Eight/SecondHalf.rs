...
fn count(board: Vec<Vec<char>>) -> u64 {
    ...
    // We also have to count the antennas as antinodes now
...
fn update_board(board: &mut Vec<Vec<char>>, center: (usize, usize), others: Vec<(usize, usize)>) {
    for (x, y) in others.iter() {
        ...
        // A multiplier as it gets further away from the center
        let mut factor: isize = 1;

        // Trying to add it a bunch of times
        loop {
            // Breaking if the multiplier gets high enough
            if factor - 1 > board.len() as isize {
                break;
            }

            let mut new_pos: (isize, isize) = (
                (center.0 as isize) + difference.0 * factor,
                (center.1 as isize) + difference.1 * factor,
            );
            ...
            new_pos = (
                (*x as isize) - difference.0 * factor,
                (*y as isize) - difference.1 * factor,
            );
