...
I actually implemented A* beforehand and switched to Dijkstra's, but I haven't changed the function names cause if it works don't touch it
...
fn a_star(board: Vec<Vec<Node>>) -> usize {
    ...
    while let Some((current_state, Reverse(_score))) = pq.pop() {
                ...
                if let Some(old_cost) = hashbrown.get_mut(&(new_pos.0, new_pos.1, current_state.2)) {
                    if new_cost < *old_cost {
                        *old_cost = new_cost;
                        ...
                        pq.push((new_pos.0, new_pos.1, current_state.2, new_cost), Reverse(new_cost));
                    }
                }
            } else {
            ...
            if let Some(old_cost) = hashbrown.get_mut(&(current_state.0, current_state.1, new_dir)) {
                if current_state.3 + 1000 < *old_cost {
                    *old_cost = current_state.3 + 1000;
                    ...
                    // Adding the updated node to the priority queue
                    pq.push((current_state.0, current_state.1, new_dir, current_state.3 + 1000), Reverse(current_state.3 + 1000));
                }
            }
        } else {
            // Adding the new value
